...
// typesafty + allows any color to be badgeColor
type BadgeColor = "#43b581" | "#faa61a" | "#ff9b9b" | (string & {})
    ...
    <div className="w-full flex items-start justify-start ">
      ...
      <div className="w-full max-w-xl">
        ...
        <div className="bg-[#2f3136] text-sm w-full rounded p-3 mb-4 mt-1.5">
          <div className="flex flex-row items-center text-nowrap justify-between mb-2">
            {badgeText ? (
              <span
                className={cn(
            ...
            <p className="text-white order-1 text-base/7 font-semibold capitalize">
              {title}
            </p>
          </div>
          {Object.entries(content).map(([key, value]) => (
            <p key={key} className="text-[#dcddde] text-sm/6">
              <span className="text-[#b9bbbe] capitalize">{key}:</span> {value}
            </p>
          ))}
